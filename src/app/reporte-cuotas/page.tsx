...
function Paginator({ page, totalPages }: { page: number; totalPages: number }) {
  ...
  // Rango visible: ventana de 5 páginas centrada en la actual
  const delta = 2;
...
export default async function ReporteCuotasPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const currentYear = new Date().getFullYear();
  const page = Math.max(1, parseInt(params.page ?? "1", 10) || 1);

  // primaryYear = año anterior al vigente (período del ciclo fiscal)
  // secondaryYear = año actual
  // previousYear = 2 años atrás (aparece en "Cartera vencida" y "Pago Anticipado")
  const listing = await getFeeReportUseCase.execute({
    primaryYear: currentYear - 1,   // 2025  (en 2026)
    secondaryYear: currentYear,      // 2026  (en 2026)
    page,
    pageSize: 20,
  });
    ...
    return (
      <main className="min-h-screen bg-[#f3ede4] px-6 py-12">
        <section className="mx-auto max-w-3xl rounded-3xl border border-[#d4c5b0] bg-white/90 p-10 shadow-xl">
          <h1 className="text-3xl font-semibold text-[#3a2a1e]">Reporte de cuotas</h1>
          <p className="mt-4 text-sm text-[#7a6050]">
            No se encontró un condominio activo o no existe el grupo de cobro ordinario.
              ...
              <table className="min-w-max border-collapse text-sm">
                ...
                <thead className="sticky top-0 z-30 shadow-sm">
                  ...
                  <tr>
                    ...
                    {/* Histórico */}
                    <th colSpan={2} className="border-b border-[#c8b8a0] bg-[#e8ddd0] px-2 py-1.5 text-center text-[9px] font-bold uppercase tracking-widest text-[#6a5040]">
                      Histórico – {previousYear}
                    </th>
                    {/* Año primary */}
                    <th colSpan={3} className="border-b border-[#b8c8e0] bg-[#dce8f0] px-2 py-1.5 text-center text-[9px] font-bold uppercase tracking-widest text-[#2a4860]">
                      Ordinarias {primaryYear}
                    </th>
                    {/* Año secondary */}
                    <th colSpan={3} className="border-b border-[#b0d0b8] bg-[#d8f0dc] px-2 py-1.5 text-center text-[9px] font-bold uppercase tracking-widest text-[#1a4828]">
                  ...
                  {vm.rows.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.id} className={`border-t border-[#e8ddd0] transition-colors hover:brightness-[0.97] ${baseBg}`}>
                        ...
                        {/* Anticipado año anterior */}
                        <td className="border-r-2 border-[#c8b8a0] px-2 py-1.5 text-center">
                          <Cell cell={row.prepaid} />
                        </td>

                        {/* Año primario — 3 celdas en un fragment con key correcto */}
                        {row.yearCells
                          .filter((yc) => yc.year === primaryYear)
                          .map((yc) => (
                            <React.Fragment key={`yN-${row.id}-${yc.year}`}>
                              <td className="border-r border-[#c8dcea] px-2 py-1.5 text-center">
                                <Cell cell={yc.annual} />
                              </td>
                              <td className="border-r border-[#c8dcea] px-2 py-1.5 text-center">
                                <Cell cell={yc.monthly} />
                              </td>
                              <td className="border-r-2 border-[#b0c8dc] px-2 py-1.5 text-center">
                                <Cell cell={yc.balance} />
                              </td>
                            </React.Fragment>
                          ))}

                        {/* Año secundario — 3 celdas */}
                        {row.yearCells
                          .filter((yc) => yc.year === secondaryYear)
                          .map((yc) => (
                            <React.Fragment key={`yN1-${row.id}-${yc.year}`}>
                              <td className="border-r border-[#b8d8be] px-2 py-1.5 text-center">
                                <Cell cell={yc.annual} />
                              </td>
                              <td className="border-r border-[#b8d8be] px-2 py-1.5 text-center">
                                <Cell cell={yc.monthly} />
                              </td>
                              <td className="border-r-2 border-[#a0c8a8] px-2 py-1.5 text-center">
                                <Cell cell={yc.balance} />
                              </td>
                            </React.Fragment>
                          ))}
                ...
                <tfoot>
                  <tr className="border-t-2 border-[#b0a090] bg-[#e8dfd0] font-bold">
                    ...
                    <td className="border-r border-[#d0c0aa] px-2 py-2 text-center"><Cell cell={vm.totalsRow.pastDue} /></td>
                    <td className="border-r-2 border-[#c8b89a] px-2 py-2 text-center"><Cell cell={vm.totalsRow.prepaid} /></td>

                    {vm.totalsRow.yearCells
                      .filter((yc) => yc.year === primaryYear)
                      .map((yc) => (
                        <React.Fragment key={`tot-yN-${yc.year}`}>
                          <td className="border-r border-[#c0d8e8] px-2 py-2 text-center"><Cell cell={yc.annual} /></td>
                          <td className="border-r border-[#c0d8e8] px-2 py-2 text-center"><Cell cell={yc.monthly} /></td>
                          <td className="border-r-2 border-[#a8c8dc] px-2 py-2 text-center"><Cell cell={yc.balance} /></td>
                        </React.Fragment>
                      ))}
                    {vm.totalsRow.yearCells
                      .filter((yc) => yc.year === secondaryYear)
                      .map((yc) => (
                        <React.Fragment key={`tot-yN1-${yc.year}`}>
                          <td className="border-r border-[#a8d8b0] px-2 py-2 text-center"><Cell cell={yc.annual} /></td>
                          <td className="border-r border-[#a8d8b0] px-2 py-2 text-center"><Cell cell={yc.monthly} /></td>
                          <td className="border-r-2 border-[#90c8a0] px-2 py-2 text-center"><Cell cell={yc.balance} /></td>
                        </React.Fragment>
                      ))}

                    <td className="border-r-2 border-[#d0b898] px-2 py-2 text-center"><Cell cell={vm.totalsRow.totalBalance} /></td>
