...
export function CondominioEditor({ initialValues, condominiumSlug }: CondominioEditorProps) {
  ...
  return (
    <Card className="shadow-layered border-transparent overflow-hidden bg-card">
      <CardHeader className="px-5 py-4 border-b border-line bg-canvas/30">
        <div className="flex items-center gap-3">
           <div className="h-9 w-9 rounded-lg bg-brand-deep flex items-center justify-center text-white">
              <Settings className="h-5 w-5" />
           </div>
           <div>
              <CardTitle className="text-sm font-black uppercase tracking-tight text-brand">Editor Maestro de Parámetros</CardTitle>
              <p className="text-[10px] font-bold text-ink-soft/40 uppercase tracking-widest leading-none mt-0.5">Control de metadatos, assets y leyendas legales</p>
           </div>
        </div>
      </CardHeader>
        ...
        <form onSubmit={onSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-line pb-1">
               ...
               <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft/60">Identidad y Formato</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="relative">
                <select 
                  ...
                  className="peer h-9 w-full rounded-md border border-line bg-card px-3 text-[13px] font-medium outline-none appearance-none focus:ring-2 focus:ring-brand-accent/20"
                >
                  <option value="1">Vertical</option>
                  <option value="2">Horizontal</option>
                  <option value="3">Mixto</option>
                </select>
                <label className="absolute left-2.5 -top-1.5 px-1 bg-card text-[10px] font-black uppercase tracking-widest text-brand-accent/60">Formato Legal</label>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-line pb-1">
               <span className="h-2 w-2 rounded-full bg-brand" />
               <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft/60">Métricas de Inventario</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-6 pt-2">
               <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-[11px] font-black uppercase text-ink-soft/60 group-hover:text-brand transition-colors">Activar Fórmula Uso Suelo</span>
               </label>
               <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-[11px] font-black uppercase text-ink-soft/60 group-hover:text-brand transition-colors">Activar Manejo VCCC</span>
               </label>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-line pb-1">
               <span className="h-2 w-2 rounded-full bg-brand" />
               <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft/60">Contenidos y Textos Legales</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
               <Textarea label="Sintesis de Aviso de Privacidad (Corta)" value={form.projectDescription} onChange={(e) => onInput("projectDescription", e.target.value)} className="min-h-[80px]" />
               <Textarea label="Cuerpo del Aviso de Privacidad" value={form.privacyNoticeText} onChange={(e) => onInput("privacyNoticeText", e.target.value)} className="min-h-[80px]" />
               <Textarea label="Pie de Recibos (Izquierdo)" value={form.footerLeft} onChange={(e) => onInput("footerLeft", e.target.value)} className="min-h-[80px]" />
               <Textarea label="Pie de Recibos (Derecho)" value={form.footerRight} onChange={(e) => onInput("footerRight", e.target.value)} className="min-h-[80px]" />
            </div>
          </div>
          ...
          <div className="space-y-4">
             <div className="flex items-center gap-2 border-b border-line pb-1">
               <span className="h-2 w-2 rounded-full bg-brand" />
               <p className="text-[10px] font-black uppercase tracking-widest text-ink-soft/60">Recursos Digitales (Assets)</p>
            </div>
              ...
              {assets.map((asset) => {
                ...
                return (
                  <div key={asset.key} className="p-3 bg-canvas/30 rounded-lg border border-line/50 flex flex-col gap-3">
                    <p className="text-[10px] font-black uppercase text-brand tracking-tighter">{normalizeAssetLabel(asset.key)}</p>
                       ...
                       {hasFile ? (
                         asset.accept.includes("image") ? (
                           ...
                           <div className="flex flex-col items-center gap-1">
                             ...
                             <span className="text-[9px] font-black text-ink-soft/40 uppercase">PDF Cargado</span>
                           </div>
                         )
                       ) : (
                         <div className="flex flex-col items-center gap-1">
                           <Upload className="h-6 w-6 text-ink-soft/10" />
                           <span className="text-[9px] font-black text-ink-soft/20 uppercase tracking-tighter">Sin archivo</span>
                         </div>
                       )}
                       {isUploading && (
                    ...
                    <div className="flex flex-col gap-2">
                       <label className="h-7 px-3 flex items-center justify-center rounded-pill bg-brand/5 border border-brand/10 text-brand text-[9px] font-black uppercase cursor-pointer hover:bg-brand transition-colors hover:text-white">
                       ...
                       {hasFile && (
                         <a href={value} target="_blank" className="h-7 px-3 flex items-center justify-center rounded-pill bg-canvas text-ink-soft/60 text-[9px] font-black uppercase gap-1.5 border border-line hover:text-brand transition-colors">
          ...
          <div className="pt-6 border-t border-line flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
               ...
               <Button 
                ...
                className="h-10 px-10 text-[11px] font-black uppercase gap-2 shadow-xl shadow-brand/20"
               >
