...
export function ExpenseEditPage({ expense, budgetConcepts, condominiumSlug, projectId }: Props) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [uploading, setUploading] = useState(false);

  const [formDate, setFormDate] = useState(toInputDate(expense.date));
  const [formConcept, setFormConcept] = useState(expense.concept);
  const [formAmount, setFormAmount] = useState(String(expense.amount));
  const [formMethod, setFormMethod] = useState(expense.paymentMethod ?? "CASH");
  const [formNotes, setFormNotes] = useState(expense.notes ?? "");
  const [formBudgetConceptId, setFormBudgetConceptId] = useState(expense.budgetConceptId ?? "");
  const [formProjectName, setFormProjectName] = useState(expense.projectName ?? "");

  const [receiptUrl, setReceiptUrl] = useState(expense.receiptUrl ?? "");
  const [receiptFile, setReceiptFile] = useState<File | null>(null);
  const [receiptPreview, setReceiptPreview] = useState<string | null>(null);
  const originalReceiptUrl = expense.receiptUrl ?? "";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-[ui-serif] text-3xl text-[#2f221a]">Editar egreso</h1>
          <p className="text-[#9a6a4a] text-sm mt-1 uppercase tracking-widest font-medium">
            {expense.budgetConceptName ?? "Sin categoría"} — {toInputDate(expense.date)}
          </p>
        </div>
      </div>
